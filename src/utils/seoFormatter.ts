...
import {
  decodeHTMLEntities,
  extractProductCountFromContent,
...
export function formatSEOText(text: string, title: string = '', category: string = '', fullContent: string = ''): string {
  ...
  const productCount = extractProductCountFromContent(fullContent, title);

  // Smart Filtering: Extract the core value proposition sentence
  ...
  processed = replaceDynamicPlaceholders(processed, { title, contentHtml: fullContent });
